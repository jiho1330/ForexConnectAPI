...
IO2GRequest *removeOrderRequest(const char *sAccountID, const char *sOrderID);
bool findOrder(const char *sOrderID, const char *sAccountID);
...
//-----------------------------------------------------------------------------------//
int __stdcall deleteOrder(const char *sAccount, const char *orderID)
{
	int ret = 0;
	char buf[400];
	...
	if (sAccount != NULL)
	{
		if (findOrder(orderID, sAccount) == false)
		{
			cb_sendMsg("Order not found\n", 2);
			return(0);
		}

		O2G2Ptr<IO2GRequest> request = removeOrderRequest(sAccount, orderID);
		if (request)
		{
			responseListener->setRequestID(request->getRequestID());
			session->sendRequest(request);
...
bool findOrder(const char *sOrderID, const char *sAccountID)
{
	...
	if (!requestFactory)
	{
		cb_sendMsg("Cannot create request factory\n", 2);
		return(orderFound);
	}
	O2G2Ptr<IO2GRequest> request = requestFactory->createRefreshTableRequestByAccount(Orders, sAccountID);
	if (!request)
	...
	if (!responseListener->waitEvents())
	{
		cb_sendMsg("Response waiting timeout expired\n", 2);
		return(orderFound);
	}
	O2G2Ptr<IO2GResponse> response = responseListener->getResponse();
	if (response)
